...
public class SplashActivity extends BaseActivity implements OnScanConfigListener {
    ...
    private void initPermission() {
        ...
        AndPermission.with(this).runtime().permission(permissionArr).onGranted(permissions -> loadMusicData()).onDenied(permissions -> againPermission()).start();
    }
    ...
    private void loadMusicData() {
        if (mSps.getInt(Constant.MUSIC_LOAD) == Constant.NUMBER_EIGHT) {
            ...
            ScannerConfigDialog.newInstance(true, this).show(getSupportFragmentManager(), "auto_config");
    ...
    private void startMusicActivity() {
        SplashActivity.this.startActivity(new Intent(SplashActivity.this, MusicActivity.class));
        finish();
    }


    @Override
    public void scanMusic(boolean isAutoScan) {
        if (isAutoScan) {
