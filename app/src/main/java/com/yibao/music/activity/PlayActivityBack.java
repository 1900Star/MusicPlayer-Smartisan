...
import com.yibao.music.databinding.PlayActivityBackBinding;
...
public class PlayActivityBack extends BasePlayActivity implements View.OnClickListener {
    private PlayActivityBackBinding mBinding;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding = PlayActivityBackBinding.inflate(getLayoutInflater());
...
//            mBinding.llSunAndDelete.setVisibility(mLyricList.size() > 2 ? View.VISIBLE : View.GONE);
            mBinding.groupLyrics.setVisibility(mLyricList.size() > 2 ? View.VISIBLE : View.GONE);
        ...
        try {
            ImageUitl.loadPic(this, url, mBinding.playingSongAlbum, R.drawable.playing_cover_lp, isSuccess -> {
                if (isSuccess) {
                    ...
                    QqMusicRemote.getSongImg(PlayActivityBack.this, mCurrentMusicInfo.getTitle(), url1 -> {
                        if (url1 == null) {
                            showAlbum(false);
                        } else {
                            Glide.with(PlayActivityBack.this).load(url1).placeholder(R.drawable.playing_cover_lp).error(R.drawable.playing_cover_lp).into(mBinding.playingSongAlbum);
                            showAlbum(true);
                        }
                    });
...
//        mBinding.rotateRl.setBackgroundColor(ColorUtil.transparentColor);/
        mBinding.groupAlwaysOn.setBackgroundColor(ColorUtil.transparentColor);
        if (mAnimator == null || mAnimatorListener == null) {
            mAnimator = AnimationUtil.getRotation(mBinding.albumCover);
...
//        mBinding.rotateRl.setOnClickListener(this);
        mBinding.groupAlbum.setOnClickListener(this);
        ...
        mBinding.ivSearchLyric.setOnClickListener(this);
...
//            screenAlwaysOnSwitch(mBinding.ivSecreenSunSwitch);
    ...
    private void showLyrics() {
        ...
        mBinding.groupLyrics.setVisibility(isShowLyrics ? View.GONE : mLyricList.size() > 2 ? View.VISIBLE : View.GONE);
        ...
        if (mLyricList.size() < Constant.NUMBER_TWO) {
            if (mCloseLyrDisposable == null) {
                mCloseLyrDisposable = Observable.timer(5, TimeUnit.SECONDS)
                        ...
                        .subscribe(aLong -> PlayActivityBack.this.showLyrics());
            }
        }
    }
